...
export default {
  setup() {
    //setup() 함수를 쓰는 순간 무조건 맨 끝에는 return 이 와야함.
    // 반응형 체크할 변수(ref , reactive) , 메서드(화살표 함수) , 컴퓨티드(자동계)
    // 리턴할 것이 많으므로 , 객체리터럴{} 로 반환해야함.

    //오른쪽에 삼선무늬를 클릭 했을때 하위 메뉴를 보이게 할지 말지를 결정
    //삼선 무늬의 상태값을 저장할 수 있는 변수 하나 선언
    // 상태값 변화되었을 때 자동으로 처리하고 싶은 부분(computed())
    // computed() 는 무조건 반환값이 있어야 함.
    //삼선무늬의 상태값이  true/false인지에 따라서 설정할 css 선택하기.
    //(삼항연산자) 조건? true일때 실행되는 코드:false일때 실행되는 코드

    // 삼선무늬를 눌렀을 때 상태값을 변경해주는 메서드를 만들기
    ...
    const changeIsNavShow = () => {
      ...
      // 누를 때마다 두 개의값이 변경하는 경우(toggle 기능)
    };
    return { state, changeIsNavShow, navClass };
    // () => {} : { }는 실행하는 코드가 1줄일때는 안써도 됨.
    // return이 없는 경우는 { }일때는 반드시 명시해야 하고, { }생략한 경우 return 생략 가능
  },
};
</script>
